 ...
 */
final class DefaultSuccess<S, F> implements Result<S, F> {

    private final S value;

    DefaultSuccess(S value) {
        this.value = value;
    }
    ...
    public Result<S, F> filter(Predicate<? super S> predicate, Function<? super S, ? extends F> failureMapper) {
        ...
        return predicate.test(this.value) ? this : new DefaultFailure<>(failureMapper.apply(this.value));
    }
